...
//			mav.getModel().putAll(bindResult.getModel());
	...
	/*
	 * 1. 관리자 강제 탈퇴 - 비밀번호에 관리자 비밀번호 입력하기 - 관리자비밀번호가 맞는 경우 해당 회원정보 db에서 삭제 - 삭제 성공 :
	 * list.shop으로 페이지 이동 - 삭제 실패 : delete.shop으로 페이지 이동 2. 본인 탈퇴 - 비밀번호에 본인 비밀번호
	 * 입력하기 - 비밀번호가 맞는 경우 회원 정보 삭제 - 삭제 성공 : 로그아웃 후 login.shop 으로 페이지 이동 - 삭제 실패 :
	 * delete.shop으로 페이지 이동
	 */
	@PostMapping("delete")
	public ModelAndView delete(User user, HttpSession session) {
		...
		try {
			...
			e.printStackTrace();
			throw new LogInException("삭제가 실패했어요!", "delete.shop?id=" + loginUser.getUserId());
		}
		return mav;
	}
}
